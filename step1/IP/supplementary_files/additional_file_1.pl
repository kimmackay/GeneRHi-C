%% Note: this program assumes a local version of the Gurobi solver
%% exists on the computer. The majority of this program formats the 
%% input file in order to pass the data to Gurobi and parses 
%% the output generated by that solver.  The program makes use of the 
%% temporary files /tmp/all.lp and /tmp/all.sol.

:- use_module(library(lists)).
:- use_module(library(csv)).
:- use_module(library(system3)).

chromosome(testMap, 1, 1, 6).
...
solve_ip(Species, Set1, Set2, M) :-
	...
	write('End\n'),
	told,
	system('gurobi_cl ResultFile=/tmp/all.sol /tmp/all.lp'),
	parse_solution.
