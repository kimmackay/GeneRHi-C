%% Note: this program assumes a local version of BlossumV
%% exists on the computer.  The majority of this program formats the 
%% input file in order to pass the data to BlossumV and parses 
%% the output generated by that solver.  The program makes use of the 
%% temporary files /tmp/all.in and /tmp/all.out.
...
genome_size(testMap, 6).

scale_factor(testMap, 1.0E1).
